...
async def process_email_async(email_id: str):
    ...
    try:
        ...
        # CRITICAL: Check if this email is a reply from a follow-up recipient - cancel their follow-ups if so
        thread_id = email_message.thread_id
        if thread_id:
...
async def handle_google_oauth_callback(code: str, state: str):
    ...
    try:
        ...
        oauth_email = result['user_info'].get('email', '')
        user_name = result['user_info'].get('name', 'Google User')
        ...
        if 'email' in result.get('authorized_services', []):
            try:
                ...
                if not existing_account:
                    ...
                    logger.info(f"✅ Auto-created Gmail email account for {oauth_email} (User: {result['user_id']})")
                else:
